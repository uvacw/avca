...
def create_features(confidence_for_binary = 0):
    ...
    traintest = pd.read_pickle('working_data/fullsample/fullsample_traintest.pkl')
...
def get_features(dataset_name, dataset):
    ...
    logmsg(str('total features in full dataset:' ) + str(len(features)))

    # Opening training dataset to get features in original dataset

    original = pd.read_pickle('../Results_ORM/working_data/machinecoding/'+dataset_name+'.pkl')

    if 'google' in dataset_name:
        original_features = [column for column in original.columns if 'google_' in column]
    elif 'microsoft' in dataset_name:
        original_features = [column for column in original.columns if 'microsoft_' in column]
    elif 'clarifai' in dataset_name:
        original_features = [column for column in original.columns if 'clarifai_' in column]
    else:
        original_features = [column for column in original.columns if ('clarifai_' in column) or ('microsoft_' in column) or ('google_' in column)]

    logmsg(str('total features in original (training) dataset: ') + str(len(original_features)))

    logmsg('total tags NOT in original (training) dataset: ' + str(len(set(features)-set(original_features))))
...
def predict_cases(dataset_name, variable, train_set, classifier_name):
    msg = '*' * 50
    logmsg(msg)
    logmsg(str('predicting cases for ' + variable + ' with ' + dataset_name))

    features = get_features(dataset_name, train_set)

    best_clf = joblib.load('best_models/'+classifier_name+'.joblibpickle') 
    
    train_set[variable+'_predicted_'+classifier_name] = best_clf.predict(train_set[features])
    train_set.to_pickle('working_data/predictions/'+classifier_name+'.pkl')
